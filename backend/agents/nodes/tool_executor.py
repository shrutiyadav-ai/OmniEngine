...
async def tool_executor_node(state: AgentState) -> dict[str, Any]:
    ...
    if not tool_name:
        ...
        return {
            ...
            "next_node": "tool_executor" if step_idx + 1 < len(plan) and plan[step_idx + 1].get("tool") else "evaluator",
        }

    logger.info("Executing tool '%s' for step %d: %s", tool_name, current_step.get("step_id"), current_step.get("description"))
    ...
    try:
        ...
        try:
            ...
            if tool:
                tool_result = await tool.execute(query=current_step.get("description", ""))
        ...
        current_step["retries"] = current_step.get("retries", 0) + 1
