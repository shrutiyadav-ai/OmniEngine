...
class CodeInterpreterTool(BaseTool):
    ...
    description = "Executes Python code in a secure, sandboxed container. Returns stdout and stderr."
    args_schema = CodeInterpreterInput

    async def execute(self, code: str, timeout_seconds: int = 30, **kwargs: Any) -> str:
        ...
        try:
            ...
            container = client.containers.run(
                ...
                volumes={
                    tmp_dir: {"bind": "/sandbox/workspace", "mode": "ro"}
                },
            ...
            try:
                result = await loop.run_in_executor(
                    None,
                    lambda: container.wait(timeout=timeout_seconds)
                )
            ...
            logger.warning("Docker sandbox not available, using subprocess/simulation fallback: %s", str(e))
